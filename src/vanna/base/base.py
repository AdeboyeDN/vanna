...
class VannaBase(ABC):
    ...
    def extract_sql(self, llm_response: str) -> str:
        ...
        # Match WITH ROLLUP clause
        sqls = re.findall(
            r"\bWITH\s+ROLLUP\b.*?;", llm_response, re.DOTALL | re.IGNORECASE
        )
        if sqls:
            sql = sqls[-1]
            self.log(title="Extracted SQL", message=f"{sql}")
        ...
        # Match WITH clause (CTEs) - Adjusted to exclude WITH ROLLUP
        sqls = re.findall(
            r"\bWITH\b(?!\s+ROLLUP).*?;", llm_response, re.DOTALL | re.IGNORECASE
        )
        if sqls:
            sql = sqls[-1]
            self.log(title="Extracted SQL", message=f"{sql}")
